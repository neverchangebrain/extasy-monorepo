...
class CareerFormSelectMenuInteraction extends BaseContinuity<CareerFormDataType, StringSelectMenuInteraction> {
  constructor(handler: ContinuityHandler<CareerFormDataType, StringSelectMenuInteraction>) {
...
const careerFormSelectMenuInteraction = new CareerFormSelectMenuInteraction(async ({ interaction, data }) => {
  ...
  const [career] = await db.select().from(careers).where(eq(careers.id, selectedValue)).limit(1);
  if (!career) {
    ...
    .setLabelComponents(
      ...
      (labelBuilder) =>
        labelBuilder
          ...
          .setDescription('Расскажите кратко о своем опыте и почему вы заинтересованы в этой вакансии')
          .setTextInputComponent((inputBuilder) =>
            inputBuilder
              .setCustomId('base2')
              .setPlaceholder('Был админом модераторов в 2019 году, познакомится с сервером изнутри')
              .setStyle(TextInputStyle.Short)
              .setRequired(true),
          ),
      (labelBuilder) =>
        labelBuilder
          ...
          .setDescription('Он поможет нам понять, когда вам будет удобно работать и общаться с командой')
